...
public class PijActor: PiCalculator{
    public override fun calculate(nrOfWorkers : Int, nrOfElements: Int, nrOfMessages: Int){
        System.out.println("JACTOR-KOTLIN - calc - (" + nrOfWorkers + "," + nrOfElements + "," + nrOfMessages + ")");
        val mailboxFactory = JAMailboxFactory.newMailboxFactory(nrOfWorkers!!)!!;
        val mailbox = mailboxFactory.createMailbox()!!
        val future = JAFuture();
        Master(nrOfElements, nrOfMessages, mailbox).send(future);
        mailboxFactory.close();
    }
}

class Master(val nrOfElements: Int, val nrOfMessages: Int,mailbox: Mailbox) : Actor<BigDecimal, Any>(mailbox){
    protected override fun processRequest(rp: RP<BigDecimal>) {
        ...
        0..(nrOfMessages-1) forEach {
            ...
            Worker(it,nrOfElements, mailbox).send(this,prp)
        }
    }
}

class Worker(val start: Int, val nrOfElements: Int,mailbox: Mailbox) : Actor<BigDecimal, BigDecimal>(mailbox){
    protected override fun processRequest(rp: RP<BigDecimal>) {
